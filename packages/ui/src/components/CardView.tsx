...
export interface CardViewProps {
   ...
   * unchanged. Opt in only for the densest mobile hand fans (Mobile Battlefield Final Polish V3):
   * at 8-10 overlapping cards the below-artwork name row became illegible visual noise (adjacent
   * cards' names smearing into each other), and the full name is already one tap away via the
   * existing preview modal / engaged-card overlay - so the compact treatment simply omits the name
   * row rather than trying to fit it, instead of rendering text nobody could read anyway. */
  compactName?: boolean;
}
