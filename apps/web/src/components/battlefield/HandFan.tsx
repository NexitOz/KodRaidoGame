...
/** Mobile-only density tiers (Mobile Battlefield Final Polish V3). Live-measured at 360/390/412/
 * 430px against a real 10-card hand: the old fixed desktop-shared numbers produced a fan
 * ~453px wide in a 390px viewport (both edge cards clipped by the viewport, ~20px left / ~42px
...
export function HandFan({
  ...
  // Mobile-only density tier (Mobile Battlefield Final Polish V3) - `DESKTOP_TIER` is used
...
interface HandCardItemProps {
  ...
  angleStepDeg: number;
  arcStepPx: number;
  /** Mobile-only, 8+ card hands (see `CardView`'s own `compactName` doc) - the full name is still
