...
## Battlefield Visual Target 3.0 (ветка `battlefield-visual-target-3`, не смержено) ✅ Done

**Презентационный редизайн экрана Battlefield в сторону премиального "ритуального арена"-облика.
Только визуал/CSS/React-компоненты — ни движок, ни игровые правила, ни баланс карт не менялись.**

- Новый `BattlefieldArena` — слоистый фон арены (тёмный камень/металл, три концентрических
  гравированных кольца, радиальные "машинные" насечки через `repeating-conic-gradient`, разрежённое
  кольцо рун Райдо `ᚱ`) собран целиком из CSS-градиентов/бордеров/инлайн-SVG-подобной разметки —
  никакого растрового мокапа, никаких захардкоженных позиций карт. Принимает `pulseKey`: при
  активации Резонанса/Трека по всей арене проходит расширяющееся кольцо (новый keyframe
  `arena-pulse`), связывая презентацию с уже существующими игровыми событиями, а не с новым UI.
- `CreatureSlot`: пустой слот теперь читается как утопленная в пол арены гравированная руническая
  плита (inset-тень + круглый глиф) вместо пунктирного прямоугольника-плейсхолдера; занятый слот
  стоит "над" этой плитой с мягким свечением у основания.
- Новый `EndTurnControl` — единственный (важно для Playwright/tutorial-спотлайта: ровно один DOM-
  элемент с `data-tutorial-target="end-turn"`) круглый тёмно-металлический элемент с античным
  золотом, с четырьмя читаемыми состояниями (свой ход/можно завершить/ожидание/обработка), заменил
  полноширинную кнопку-таблетку.
- `ConductorPanel`: облицовка "металлической пластины, вмонтированной в арену" (тонкая золотая
  рамка, угловые заклёпки), функциональное содержимое (HP-бар, EnergyPips, aria-label) не менялось.
- `RuneZone`: активная руна получила медленно вращающееся гравированное кольцо (новый keyframe
  `rune-rotate`, 18s linear infinite) — читается как инкрустированная геометрия, а не спиннер
  загрузки; гейтится в Low Data Mode и `prefers-reduced-motion` по уже существующему паттерну.
  `TrackZone` уменьшен с крупного центрального попапа (карта+имя+8 波-баров) до компактной таблетки
  с миниатюрной волной — "дотягивается до всей арены" теперь через `BattlefieldArena`'s `pulseKey`.
- `MatchBoard`: половина соперника отрисовывается с лёгким CSS `scale` от верхнего края для глубины
  (никакого 3D, как и требовалось); контейнер расширен на `xl`-брейкпоинте, чтобы меньше впустую
  тратить место на широких десктопах в пределах уже существующего `max-w-5xl` общего лэйаута.
- Найден и исправлен реальный баг при живом тестировании на мобильном вьюпорте: страница матча
  могла открыться с уже прокрученным вниз состоянием (артефакт клиентского перехода с `/play`),
  из-за чего панель Проводника соперника и шапка хода были скрыты выше видимой области. Добавлен
  одноразовый скролл к началу страницы после первой загрузки состояния матча — на всех трёх
  экранах матча (`/play/[matchId]`, `/pvp/[matchId]`, `/tutorial/[matchId]`).
- Полный прогон: **340/340 unit-тестов зелёных** (game-server 147, web 32, worker 24, game-engine
  88, shared 49), lint/typecheck/build чисты по всем воркспейсам. Ручной прогон Playwright e2e
  (tutorial-fpx/starter-deck-provisioning/player-progression/visual-polish/nav-and-layout,
  390×844) — 16/19 пройдено; оставшиеся 3 падения (`Tutorial driver stalled on step
  ATTACK/RESONANCE`) воспроизведены один-в-один на чистом `main` без единого изменения этой фазы —
  это существующая нестабильность generic-водителя обучения в `e2e/helpers.ts` (случайное
  затягивание матча из-за очерёдности карт в руке бота), не регрессия Battlefield 3.0.
- `GAMEPLAY CHANGED: NO` / `CARD BALANCE CHANGED: NO` / `GAME ENGINE SOURCE CHANGED: NO`.
  Канонический ростер 40 карт не затронут.

## Battlefield 3.1 + 3.2 (та же ветка `battlefield-visual-target-3`, не смержено) ✅ Done

**Второй визуальный проход по прямому запросу владельца: приблизить карточки/HUD/атмосферу арены
к присланному reference-концепту (только стиль/композиция — данные карт из БД не менялись,
никаких новых выдуманных полей/способностей) и отдельным заходом — усилить атмосферу арены
(свет, материал, мистические эффекты). Только визуал, как и в 3.0.**

**3.1 — премиальная рамка карт + HUD-медальоны** (коммит `23341f9`):
- `CardView` (общий компонент — рука, поле, коллекция, дек-билдер): двойная гравированная рамка,
  шестиугольные "самоцветные" бейджи стоимости/атаки/здоровья (тот же `clip-path`-шестиугольник,
  что и в HUD ниже), фракционная подсветка подвала карты (`bgSoftClass` в `factions.ts`), угловые
  золотые/красные акценты для Legendary/Raido, ряд "пипсов" редкости рядом с текстовой меткой.
- `HandFan`: карты в руке переведены с размера `xs` на `sm` (92px → 140px) — читаются как ценные
  предметы, а не миниатюры; шире веер/нахлёст, более выраженное красное свечение у выбранной карты.
- `CardBack`: тот же язык двойной рамки + угловых засечек, что и у лицевой стороны.
- `ConductorPanel`: шестигранный медальон-аватар вместо круга, угловые скобки вместо точек,
  амбиентное свечение к краю арены; на `sm`+ экранах — крупный шестигранный медальон с числом HP
  рядом с уже существующими баром/пипсами энергии (не заменяет их).
- `EndTurnControl`: перекроен в шестиугольник с двумя тихими кольцами-отголосками вокруг (тот же
  мотив гравированных колец, что у арены) — читается как "врезанный в кольцевую систему стола",
  а не отдельная кнопка. `data-tutorial-target="end-turn"` остаётся ровно на одном DOM-элементе.
- `CreatureRow`: 5 слотов теперь выстроены по мягкой дуге (`curve="up"` для ряда соперника,
  `curve="down"` для игрока) вместо плоской линии сетки — эхо круглых колец арены.
- `ResonancePulse`/`RuneZone`/`TrackZone`: кольца резонанса крупнее + постоянное (не только при
  триггере) амбиентное свечение, масштабируемое по tier; руны крупнее; таблетка трека получила
  цветной боковой акцент и подпись "ТРЕК" для более "статусного" вида.

**3.2 — атмосфера арены** (коммит `e1c39f4`, только `BattlefieldArena.tsx` + новые gated-анимации):
- Ключевой свет: мягкий верхний источник (`radial-gradient` + новый keyframe `arena-breathe`,
  6s ease-in-out infinite) — арена "дышит", а не освещена плоско.
- Рельефные кольца: у всех трёх концентрических колец добавлен `inset box-shadow`
  (верхний блик + нижняя тень), читаются как резное/чеканное металл, а не плоский контур.
  Общая контрастность виньетки на краях арены увеличена (второй радиальный слой поверх
  существующего токена `bg-raido-vignette`), не затемняя читаемый центр.
  Текстура зерна камня: существующий токен `bg-rune-noise` наложен на пол арены (`opacity-[0.15]`).
- Environmental storytelling: две едва заметные тёмные "колонны"-силуэта по краям арены (чистые
  CSS-градиенты, не литеральная архитектура — нет ассетов для неё) намекают, что арена находится
  внутри реальной структуры.
- Мистическая атмосфера: 5 медленно плывущих вверх и затухающих "искр"/магических частиц
  (новый keyframe `ember-drift`, 9s ease-in-out infinite, со сдвигом по `animationDelay`),
  независимо от фактических триггеров Резонанса/Рун — фоновое "живое заклинание" в состоянии покоя.
- Оба новых keyframe (`arena-breathe`, `ember-drift`) — гейтятся под Low Data Mode и
  `prefers-reduced-motion` по уже существующему паттерну (`globals.css`).

**Регрессия**: lint/typecheck/build чисты по всем воркспейсам; **340/340 unit-тестов зелёных**
(без изменений — фазы затрагивали только React/CSS-компоненты представления). Живая визуальная
проверка (Playwright-скриншоты, 1440×900 и 390×844): подтверждены шестигранные бейджи/медальоны,
рельефные кольца, зерно камня, дугообразные ряды существ, отсутствие горизонтального переполнения
на обоих вьюпортах. Полный e2e-прогон прерывался дважды перезапуском песочницы (окружение
разработки теряется, ветка и коммиты — нет, они уже запушены); в изолированном повторном прогоне
`player-progression.spec.ts` первый тест (полный детерминированный PvE-матч против бота) прошёл
за 1.7м без ошибок — то же самое сценарий, что ранее ошибочно выглядел "зависшим" в общем прогоне
из-за холодного кеша Next.js dev-сервера после перезапуска площадки, а не из-за регрессии кода.
`GAMEPLAY CHANGED: NO` / `CARD BALANCE CHANGED: NO` / `GAME ENGINE SOURCE CHANGED: NO`.

## Battlefield 3.3 (та же ветка `battlefield-visual-target-3`, не смержено) ✅ Done

**Финальный арт-дирекшн проход по прямому запросу владельца: только визуальная насыщенность и
environmental storytelling пустого десктопного пространства арены — раскладка, геймплей, карточная
логика, селекторы и тест-хуки не менялись.** Затронуты 4 файла, все чисто декоративные добавления
(`aria-hidden`/`pointer-events-none`, без нового DOM для интерактивных элементов):

- `BattlefieldArena.tsx`: усилен "heart"-glow в центре арены (та же точка, где сидит
  `ResonancePulse`); добавлены desktop-only (`hidden lg:block`) "рунические энергетические пути" —
  5 асимметричных тонких золотых лучей-проводников, расходящихся от центра к краям арены (переиспользуют
  существующую gated-анимацию `animate-pulse-rune`, новых keyframes не добавлено); добавлена
  desktop-only внешняя "рамка чаши" с угловыми скобками вокруг всего прямоугольника борда, чтобы
  пустой отступ вокруг круглой арены читался как спроектированная камера, а не как лишнее поле.
- `CreatureSlot.tsx`: пустой слот существа переработан из плоского тёмного круга в "ритуальную
  платформу" — тот же рельефный (rim-lit embossed metal) язык, что и у колец арены, плюс тонкое
  кольцо гравированных засечек (repeating-conic-gradient) и тёплое амбиентное свечение у основания.
- `ResonancePulse.tsx`: под кольцом Резонанса добавлен небольшой рельефный "постамент" (та же
  rim-lit-схема), чтобы центр арены читался как её укоренённый энергетический источник, а не как
  бейдж на прозрачном фоне.
- `RuneZone.tsx`: ряд активных рун получил едва заметную рельефную подложку, объединяющую его с
  материальным языком арены, вместо иконок, плавающих без опоры.
- `TrackZone.tsx` не менялся — его статус-плашка (цветной акцент + подпись "ТРЕК") признана уже
  достаточно интегрированной по итогам 3.1.

**Регрессия**: lint/typecheck/build чисты по всем воркспейсам; **340/340 unit-тестов зелёных**
(без изменений — фаза затрагивала только React/CSS-компоненты представления, `git diff --stat`
подтверждает ровно 4 изменённых файла). Живая проверка через Playwright на реальном PvE-матче
(1440×900 и 390×844): подтверждено отсутствие горизонтального переполнения на обоих вьюпортах;
пустые слоты существ показывают чётко читаемый гравированный узор вместо плоских кругов; десктопная
внешняя рамка и кольца рендерятся (геометрия и вычисленный `opacity` подтверждены через
`getBoundingClientRect`/`getComputedStyle` в браузере) — по дизайну они очень тонкие ("subtle" было
явным требованием владельца), поэтому энергетические пути были один раз усилены по интенсивности
(`opacity-[0.16]` → `opacity-[0.4]`, толще линия, добавлено лёгкое свечение) после визуальной
проверки скриншотов, прежде чем принять текущий уровень как соответствующий духу "subtle". Мобильная
раскладка (390×844) визуально подтверждена компактной и читаемой: desktop-only декоративные
элементы корректно скрыты, "ритуальные платформы" и Resonance-постамент не создают визуального шума
на маленьком экране.
`GAMEPLAY CHANGED: NO` / `CARD BALANCE CHANGED: NO` / `GAME ENGINE SOURCE CHANGED: NO` /
`SELECTORS OR TEST HOOKS CHANGED: NO`.

## Battlefield 3.4 (та же ветка `battlefield-visual-target-3`, не смержено) ✅ Done

**По прямому запросу владельца: сделать Резонанс безошибочно узнаваемым визуальным "сердцем" арены
на десктопе — крупнее слоёв колец резонанса, управляемое радиальное свечение, едва заметная
waveform/рунная геометрия, и чётко видимые (без brightening/heatmap/DOM-инспекции) энергетические
проводники к рядам существ — плюс лёгкое усиление внешней рамки чаши. Раскладка/mobile — не
трогать, геймплей/селекторы/тест-хуки/размерная логика — не менять.**

**Критическая находка (не баг этой фазы, а обнаруженная ею давняя проблема с 3.0): почти весь
декоративный слой `BattlefieldArena` — кольца, рунный круг, зерно, всё, что накладывается на область
рядов существ — фактически НИКОГДА не было видно на экране, с самого 3.0.** Причина — `-z-10` на
корневом div арены: по правилам CSS-стекинга элемент с отрицательным z-index красится НИЖЕ любого
соседа с `position: relative`/`absolute` и z-index:auto в том же контексте стекинга — а такие соседи
(строки существ, их "ритуальные платформы" с собственным непрозрачным фоном) используются почти
везде в `MatchBoard`. DOM-порядок (арена — первый ребёнок) тут не помогает: буква стекинга важнее
порядка. Раньше это никогда не обнаруживалось, потому что "видимость" декора всегда проверялась
только визуальным осмотром сжатых скриншотов — сырые пиксели в области перекрытия с рядами существ
никогда не сэмплировались напрямую. Этот раз потребовал именно этого (прямое требование владельца
"видно на нетронутом скриншоте"): диагностика через `getBoundingClientRect`/`getComputedStyle`
подтверждала "всё верно" (правильные размеры/цвета/opacity), но прямой сэмплинг пикселей PNG на тех
же координатах показывал чистый фон без единого оттенка — а контрольный полностью непрозрачный
красный `<div>`, добавленный для диагностики, тоже не показывался, пока не был убран `-z-10`.
**Исправление**: убран `-z-10` у `BattlefieldArena` и у двух идентично сломанных light-shift div'ов
в `MatchBoard.tsx` (та же "тёплое/холодное" подсветка активного игрока из 3.0, тоже никогда не
рендерилась) — оставлен просто `position: absolute` без z-index; поскольку `BattlefieldArena`
рендерится первым ребёнком в дереве, DOM-порядок теперь корректно держит её позади всего
интерактивного контента, без необходимости в отрицательном z-index. Побочный эффект: весь
предыдущий (3.0-3.3) декор арены — кольца, рунный круг, зерно, ember-частицы — теперь ВПЕРВЫЕ
реально виден и на мобильной раскладке тоже (не только на десктопе), что признано желательным
улучшением, а не регрессией (запрошенная "минимальность изменений на mobile" соблюдена — ни один
mobile-затрагивающий класс/значение не менялся, изменилась только видимость уже существовавшего кода).

**Новые слои** (`BattlefieldArena.tsx`, всё в новых desktop-only `hidden lg:block` блоках, не
затрагивают существующие 3.0-3.3 слои):
- Resonance-heart кластер: управляемое радиальное свечение (мягкий большой ореол), два новых слоя
  колец (средний красный + внутренний золотой, оба намеренно тоньше и тусклее самого первого
  прохода — после того, как фикс стекинга сделал их видимыми, исходная интенсивность оказалась
  слишком яркой/резкой для "elegant"), и статичное "paused equalizer" кольцо коротких радиальных
  штрихов переменной длины вокруг центра — музыкальный waveform-мотив, тема Kod Raido.
- Энергетические проводники к рядам существ: 6 симметричных лучей (2 коротких вертикальных — по
  реально измеренному зазору до каждого ряда, ~56–82px на 1440×900 — плюс 4 более длинных
  диагональных к флангам), переиспользуют существующую gated-анимацию `animate-pulse-rune`
  (минимальный уровень opacity в её keyframe — 0.55, так что видимость не зависит от фазы анимации).
  Специально размещены в разметке ПОСЛЕ виньетки/угловго затемнения (не среди слоёв 3.0-3.3), чтобы
  читаться как источник света поверх затемнения, а не быть скрытыми под ним.
- Лёгкая тёплая засветка по флангам (desktop-only, чистый свет/геометрия, не новый UI) — заполняет
  плоское чёрное поле у панелей Проводника.
- Внешняя рамка чаши чуть усилена (opacity бордера/скобок повышен, было признано слишком тонким).

**Регрессия**: lint/typecheck/build чисты по всем воркспейсам; **340/340 unit-тестов зелёных**
(без изменений). `git diff --stat` — ровно 2 изменённых файла (`BattlefieldArena.tsx`,
`MatchBoard.tsx`, только удаление `-z-10` + новые decorative-only блоки). Видимость энергетических
проводников и колец подтверждена НАПРЯМУЮ — сырыми пикселями нетронутого (не осветлённого)
скриншота на координатах, полученных через `getBoundingClientRect`, а не визуальным впечатлением от
превью. Интенсивность после первого прохода оказалась излишне яркой (насыщенно-красное кольцо,
почти сплошной золотой крест) — уменьшена (opacity колец/свечения примерно вдвое, проводники
переведены с "лазерного" градиента на мягкий золотой) до баланса "чётко видно, но элегантно".
Мобильная раскладка (390×844) визуально подтверждена компактной; desktop-only блоки корректно
скрыты; общий эффект фикса стекинга на mobile — атмосфера арены стала явно богаче (кольца/зерно
теперь видны), без нового визуального шума.
`GAMEPLAY CHANGED: NO` / `CARD BALANCE CHANGED: NO` / `GAME ENGINE SOURCE CHANGED: NO` /
`SELECTORS OR TEST HOOKS CHANGED: NO` / `SIZING LOGIC CHANGED: NO`.
